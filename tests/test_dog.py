...
def test_singl_req(fix_get_dog_host):
    '''Проверяем запрос на получение одной фотографии. Проверяем схему json'''
    respons = requests.get('{}{}'.format(fix_get_dog_host, 'breeds/image/random'))
    shema = {
        'type': 'object',
        'property':
            {
...
def test_random_req(fix_get_dog_host, dog_count):
    '''Проверяем запрос на получение нескольких фотографий. Проверяем количество фотографий в поле message'''
    respons = requests.get('{}{}'.format(fix_get_dog_host, 'breeds/image/random/' + str(dog_count)))
...
def test_sub_breeds(fix_get_dog_host, dog_breed):
    '''Проверяем запрос на получение списка подпород собак'''
    respons = requests.get('{}{}'.format(fix_get_dog_host, 'breed/' + dog_breed[0] + '/list'))
    message = respons.json()['message']

    assert message == dog_breed[1]


def test_status_code(fix_get_dog_host):
    '''Проверяем статус запроса'''
    respons = requests.get('{}{}'.format(fix_get_dog_host, 'breeds/image/random'))
    assert respons.status_code == 200


def test_status_param(fix_get_dog_host):
